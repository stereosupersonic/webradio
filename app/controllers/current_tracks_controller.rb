class CurrentTracksController < ApplicationController
  def index
    ...
    Rails.logger.info "#{@station.name} current_track_by: steam #{current_track_by_steam}" if current_track_by_steam
    @current_track = current_track_by_steam
    @current_track ||= RadioboxLastTrack.new(url: @station.radio_box_url).call
    Rails.logger.info "#{@station.name} current_track_by: radiobox #{@current_track}" if @current_track && !current_track_by_steam
    Rails.logger.info "#{@station.name} no current_track_by" unless @current_track
